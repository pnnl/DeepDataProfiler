...
def test_mul_div_raises():
    ...
    assert (
        str(excinfo.value)
        == "Can only divide by int or float. Received type <class 'lucent.optvis.objectives.Objective'>"
    )
    with pytest.raises(Exception) as excinfo:
        objective = objectives.channel("mixed4a", 0) * objectives.channel("mixed4a", 0)
    assert (
        str(excinfo.value)
        == "Can only multiply by int or float. Received type <class 'lucent.optvis.objectives.Objective'>"
    )
