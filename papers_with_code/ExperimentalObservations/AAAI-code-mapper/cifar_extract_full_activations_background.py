...
# -*- coding: utf-8 -*-
"""
Created on Tue Sep 28 16:36:29 2021
...
def get_background_masks(indices, data, iter_num = 2):
    print("get foreground masks...")
    indices_to_mask = {}
    for idx in indices:
        if idx%1000 == 0:
            print(idx)
        img_i = data[idx,:,:,:]
        img_i_new = background_detection(img_i, iter_num=iter_num)
        indices_to_mask[idx] = cv2.cvtColor(img_i_new, cv2.COLOR_BGR2GRAY)
    return indices_to_mask

def background_detection(img, iter_num=3):
    mask = np.zeros(img.shape[:2], dtype="uint8")
    ...
    mask2 = np.where((mask == cv2.GC_BGD)|(mask == cv2.GC_PR_BGD), 1, 0).astype('uint8')
    # mask2 = np.where((mask == cv2.GC_BGD), 0, 1).astype('uint8')
    ...
    for layer in layers_name_selected:
        ...
        for i in range(num_classes):
            ...
            for j in range(batch_size):
                ...
                img_ij_fg = background_detection(img_ij, iter_num=5)
        ...
        df_5_dir = "../datasets/cifar10_full_bg_5_df/"
        df_1_dir = "../datasets/cifar10_full_bg_1_df/"
        ...
        layer_activations_5_df.to_csv(df_5_dir+"train_full_bg_5_"+layer+".csv", index=False)
        ...
        layer_activations_1_df.to_csv(df_1_dir+"train_full_bg_1_"+layer+".csv", index=False)
