...
# -*- coding: utf-8 -*-
"""
Created on Tue Sep 28 04:48:38 2021
...
if __name__ == '__main__':
    ...
    # img_dir = f'../activations/{DATASET.lower()}/resnet18_custom_aug/full_activations_imgs/'
    img_dir = f'../activations/{DATASET.lower()}/resnet18_custom_aug/full_activations_tmp/'
    ...
    with open("../datasets/activation_norm_dict.json") as f:
        activation_norm_dict = json.load(f)
    ############## get weights #####################
    names = ['airplane',
      'automobile',
      'bird',
   ...
   # 'layer3.1.bn2', 'layer4.0.bn1', 'layer4.0.bn2', 'layer4.1.bn1', 'layer4.1.bn2']
    ...
    # layers_name = ['layer3.0.bn1', 'layer3.0.bn2', 'layer3.1.bn1', 'layer3.1.bn2',
    #                 'layer4.0.bn1', 'layer4.0.bn2', 'layer4.1.bn1', 'layer4.1.bn2']
    # print(train.data.shape)
    # img_i = train.data[48580,:,:]
    # print(img_i)
    layers_name = ['layer4.1.bn2']
    
    bn2conv = {}
    # for i in range(len(mask_activations.keys())):
    for i in range(15, len(mask_activations.keys())):
        conv_key = list(mask_activations.keys())[i]
        # bn_key = layers_name[i]
        bn_key = layers_name[i-15]
        bn2conv[bn_key] = conv_key
    
    plt_img_path = '../figs/perturbation/'
    for layer in layers_name:
        ...
        for i in range(num_classes):
            ...
            layer_imgs_i = read_activation(os.path.join(img_dir, f'label{i}.hdf5'), "imgs")
            predictions_i = read_activation(os.path.join(activation_dir, f'label{i}_{sigma}.hdf5'), "predictions")
            predictions_i = np.array(['true' if p == i else 'false' for p in predictions_i]).reshape(-1,1)
            layer_imgs_i_noises = read_activation(os.path.join(activation_dir, f'label{i}_{sigma}.hdf5'), "imgs")
            ...
            for j in range(batch_size):
                ...
                # img_ij = inv_normalize(torch.tensor(np.einsum('ijk->jki', img_ij)), norm_mean, norm_std)
                # if j%1000 == 0:
                #     print(j)
                #     img_ij_tmp = inv_normalize(torch.tensor(img_ij), norm_mean, norm_std).permute(1, 2, 0)
                #     img_ij_noises = layer_imgs_i_noises[j,:,:,:]
                #     img_ij_noises = inv_normalize(torch.tensor(img_ij_noises), norm_mean, norm_std).permute(1, 2, 0)
                #     plt.figure(figsize=(5, 5), dpi=500)
                #     plt.imshow(img_ij_tmp)
                #     plt.savefig(plt_img_path+names[i]+str(j//1000)+'.png')
                #     plt.close()
                #     plt.figure(figsize=(5, 5), dpi=500)
                #     plt.imshow(img_ij_noises)
                #     plt.savefig(plt_img_path+names[i]+str(j//1000)+'_'+str(sigma)+'.png')
                #     plt.close()
                ...
                predictions_i_1.append(predictions_i[j])
                predictions_i_5 += [predictions_i[j]] * 5
            ...
            predictions_i_1 = np.array(predictions_i_1).reshape(-1,1)
            predictions_i_5 = np.array(predictions_i_5).reshape(-1,1)
            layer_activations_i_5 = np.hstack([label_i_5, predictions_i_5, layer_activations_i_5])
            layer_activations_i_1 = np.hstack([label_i_1, predictions_i_1, layer_activations_i_1])

            layer_activations_5.append(layer_activations_i_5)
            layer_activations_1.append(layer_activations_i_1)
            
            print("Prediction accuracy for", names[i], "top1:", np.sum(predictions_i_1=='true')/len(predictions_i_1))
            print("Prediction accuracy for", names[i], "top5:", np.sum(predictions_i_5=='true')/len(predictions_i_5))
        ...
        df_5_dir = "../datasets/cifar10_full_fg_5_df_noises/"
        df_1_dir = "../datasets/cifar10_full_fg_1_df_noises/"
        ...
        layer_activations_5_df.to_csv(df_5_dir+"train_full_fg_5_"+layer+"_"+str(sigma)+".csv", index=False)
        ...
        layer_activations_1_df.to_csv(df_1_dir+"train_full_fg_1_"+layer+"_"+str(sigma)+".csv", index=False)
